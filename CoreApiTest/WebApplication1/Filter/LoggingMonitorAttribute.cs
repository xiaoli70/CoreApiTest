...
public sealed class LoggingMonitorAttribute : Attribute, IAsyncActionFilter, IAsyncPageFilter, IOrderedFilter
{
    ...
    private List<string> GenerateReturnInfomationTemplate(Utf8JsonWriter writer, dynamic resultContext, MethodInfo method, LoggingMonitorMethod monitorMethod)
    {
        ...
        // 获取返回值阈值
        var threshold = GetReturnValueThreshold();
        if (threshold > 0)
        {
    ...
    /// <param name="monitorMethod"></param>
    /// <returns></returns>
    private bool CheckIsSetWithReturnValue()
    {
        return true;
    }
    ...
    /// <param name="monitorMethod"></param>
    /// <returns></returns>
    private bool CheckIsSetJsonIndented()
    {
        return true;
    }
    ...
    /// <param name="monitorMethod"></param>
    /// <returns></returns>
    private int GetReturnValueThreshold()
    {
        return 500;
    }
    ...
    {
        return ContractResolverTypes.CamelCase;
    }
    ...
    private async Task MonitorAsync(MethodInfo actionMethod, IDictionary<string, object> parameterValues, FilterContext context, dynamic next)
    {
        ...
        // 配置 JSON 格式化行为，是否美化
        jsonWriterOptions.Indented = CheckIsSetJsonIndented();
        ...
        // 获取客户端 IPv4 地址
        var remoteIPv4 = httpContext.Connection.LocalIpAddress?.MapToIPv4()?.ToString();
        ...
        // 获取请求的 Url 地址
        var requestUrl = Uri.UnescapeDataString(httpRequest.Headers["Referer"].ToString());
        ...
        var isValidationException = true;
       ...
       var logger = httpContext.RequestServices.GetRequiredService<ILogger<LoggingMonitor>>();
        ...
        // 获取最终写入日志消息格式
        // var finalMessage = GetJsonBehavior(JsonBehavior, monitorMethod) == Net6Api.Domain.JsonBehavior.OnlyJson ? jsonString : monitorMessage;
        var finalMessage = monitorMessage;
